...
class VOCDataSet(Dataset):
    ...
    def __init__(self,imgs_dir="./VOC2007/Train/JPEGImages", annotations_dir="./VOC2007/Train/Annotations",
                 img_size=448, S=7, B=2, ClassesFile="../VOC2007/Train/class.data",
                 label_smooth_value = 0.05): # 图片路径、注解文件路径、图片尺寸、每个grid cell预测的box数量、类别文件
        self.transform = transform.Compose([
            transform.ToTensor(), # height * width * channel -> channel * height * width
            transform.Normalize(mean=(0.5,0.5,0.5),std=(0.5,0.5,0.5))  #归一化后.不容易产生梯度爆炸的问题
        ])
        ...
        self.S = S
        self.B = B  #每个grid cell预测的box数量
        self.grid_cell_size = self.img_size / self.S   #一个网格有几个像素
    ...
    def getGroundTruth(self):
        ...
        for annotation_file in self.annotation_path:
            ...
            for object_xml in objects_xml:
                ...
                ground_box = list([centerX / self.grid_cell_size - indexCol,  #相对于网格的位置
                                   centerY / self.grid_cell_size - indexRow,
